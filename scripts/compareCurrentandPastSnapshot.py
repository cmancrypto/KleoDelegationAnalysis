...
def main(): 
    ...
    print(f"addresses in both snaps had : {comparisonGrowthSum} in first snap and then  {comparisonSumValues}in 2nd - this is growth of {round(comparisonGrowthSum/comparisonSumValues,1)*100}%")
    print(f"there were {len(dfHistoricalDelegators)} delegators in first snap and there's now {len(comparisonDelegatorFrame)} of these delegators left")
    print(f"delegators from our first snap make up {round((comparisonSumValues/sumCurrent)*100,1)} % of our current delegators")
