import delegationsOnOtherChains
import csv
import pandas as pd
...
def getDelegatorsPublicKeysToCSV(sourcechain):
    dfDelegators=delegationsOnOtherChains.getDelegatorsAndConvert(sourcechain)
    response=delegationsOnOtherChains.queryGetAllAccounts(sourcechain)
...
def importCSVCheckBalances(chaintoanalyse):
    ...
    allAddresses=delegationsOnOtherChains.queryGetAllAccounts(chaintoanalyse)["accounts"]
    accountsOnChain=[]
    ...
    chainBalancesByAddress=delegationsOnOtherChains.queryDelegatedBalancesByAddressListAPI(comparison_addresses,chaintoanalyse)
